#ifndef ESP32_BLE_COMBO_GAMEPAD_H
#define ESP32_BLE_COMBO_GAMEPAD_H
...
class BleComboGamepad : public HIDInputDevice
{
private:
  BLECharacteristic* devHook;

  uint8_t buttons[16];
  uint8_t numberOfButtonsBytes;
  int reportSize;
  int16_t hat1;
  void sendReport();
  uint8_t id;
  int hidDescriptorSize;
  int numberOfButtons;
  int numberOfHatButtons;
  bool hasXAxis, hasYAxis, hasZAxis, hasRxAxis, hasRyAxis, hasRzAxis;
  bool hasSlider1, hasSlider2, hasRudder, hasThrottle, hasAccelerator, hasBreak, hasStearing;

public:
  BleComboGamepad(int inNumberOfButtons, int inNumberOfHatButtons) : hidDescriptorSize(0), numberOfButtons(inNumberOfButtons), numberOfHatButtons(inNumberOfHatButtons), hasXAxis(false), hasYAxis(false), hasZAxis(false), 
    hasRxAxis(false), hasRyAxis(false), hasRzAxis(false), hasSlider1(false), hasSlider2(false), 
    hasRudder(false), hasThrottle(false), hasAccelerator(false), hasBreak(false), hasStearing(false) {};
  ...
  virtual void createInputDev(BleComboInput* handler);
...
#endif // ESP32_BLE_COMBO_GAMEPAD_H
